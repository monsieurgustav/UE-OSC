...
void UOscReceiverComponent::BeginPlay()
{
        ...
        (!GetOwner() || !GetOwner()->HasAnyFlags(RF_ClassDefaultObject)))
    {
        ...
        if(GetOwner())
        {
            UE_LOG(LogUE_OSC, Verbose, TEXT("Registering component %s of %s"),  *GetName(), *GetOwner()->GetName());
        }
        else
        {
            UE_LOG(LogUE_OSC, Verbose, TEXT("Registering component %s"), *GetName());
        }
    }
}
...
void UOscReceiverComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
        ...
        (!GetOwner() || !GetOwner()->HasAnyFlags(RF_ClassDefaultObject)))
    {
        ...
        if(GetOwner())
        {
            UE_LOG(LogUE_OSC, Verbose, TEXT("Unregistering component %s of %s"),  *GetName(), *GetOwner()->GetName());
        }
        else
        {
            UE_LOG(LogUE_OSC, Verbose, TEXT("Unregistering component %s"), *GetName());
        }
    }
